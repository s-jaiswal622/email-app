let emails = [];
let selectedEmailId = null;

function saveEmails() {
  localStorage.setItem('emails', JSON.stringify(emails));
}

function loadEmails() {
  const stored = localStorage.getItem('emails');
  if (stored) {
    emails = JSON.parse(stored);
  } else {
    emails = [
      {
        id: 1,
        sender: "alice@example.com",
        subject: "Meeting at 3PM",
        body: "Let's meet in the conference room at 3PM.",
        timestamp: "2025-04-28 10:00 AM",
        read: false
      },
      {
        id: 2,
        sender: "bob@example.com",
        subject: "Lunch Invitation",
        body: "Would you like to grab lunch tomorrow?",
        timestamp: "2025-04-27 2:30 PM",
        read: false
      },
      {
        id: 3,
        sender: "carol@example.com",
        subject: "Project Update",
        body: "The project is on track and will be done by Friday.",
        timestamp: "2025-04-26 5:00 PM",
        read: false
      }
    ];
    saveEmails(); // Save default data if nothing found
  }
}
